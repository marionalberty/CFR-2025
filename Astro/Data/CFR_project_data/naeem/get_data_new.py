...
z = [0, 1]
snaps = [99, 50] #corresponding simulation timestamps
h = 0.704
...
for i in range(len(snaps)):
    subhalos = il.groupcat.loadSubhalos(basePath,snaps[i],fields=['SubhaloSFR', 'SubhaloMassType'])
    print('read in subhalos')

    #limit the galaxies we look at based on stellar mass
    stellar_mass = subhalos['SubhaloMassType'].T[4]
    in_mass_range = (stellar_mass > (10**8 / 1e10) * h) & (stellar_mass < (10**12/ 1e10) * h)
    stellar_mass = stellar_mass[in_mass_range] * 1e10 / h
    ...
    last_major_snap = last_major_snap[in_mass_range]
    print('starting time calculation')
    last_major_time = [time(snapnum2z(snap)) for snap in last_major_snap]

    #save data
    df = pd.DataFrame({'Time Since Last Major Merger [Gyr]':last_major_time, 'Stellar Mass [Msun]':stellar_mass, 'Star Formation Rate [Msun/year]':sfr})
    df.to_csv('new_info_{}.csv'.format(z[i]), index=None)
    print('saved data')